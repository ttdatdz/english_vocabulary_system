...
import "../CreateToeicQuestion/CreateToeicQuestion.scss";

const LETTERS = ["A", "B", "C", "D", "E"];

export default function CreateGroupToeicQuestion(props) {
  const {
    open,
    onClose,
    draftQuestion,
  ...
  const options = Array.isArray(draftQuestion.options)
    ...
    >
      <div style={{ height: "500px", overflow: "auto" }}>
        ...
        <div className="question-form__group">
          ...
          <div className="question-form__media-section">
            ...
            {(serverImages.length > 0 || localPreviews.length > 0) && (
              <div className="question-form__image-list">
                {serverImages.map((url, idx) => (
                  <div
                    key={`server-${idx}`}
                    className="question-form__image-item"
                  >
                    <img
                      src={url}
                      alt={`server-${idx}`}
