...
export default function PikachuPractice() {
  ...
  useEffect(() => {
    ...
    const seedPairIds = idsShuffled.slice(
      0,
      Math.min(targetPairs, idsShuffled.length)
    );
    ...
    const fillAsDistractor = () => {
      ...
      while (empties.length && guard < totalCells * 3) {
        ...
        const wouldCompletePair =
          active.has(card.id) &&
          initial.some((t) => t && t.pairId === card.id && t.lang !== side);
  ...
  function placeNewPairPrefer(indicesPrefer, currentBoard, currentActive) {
    ...
    if (!remainingIds.length)
      return { board: currentBoard, active: currentActive };

    const pickId = remainingIds[0];
    const card = allCards.find((c) => c.id === pickId);
    const empties = emptyIndices(currentBoard);
    if (empties.length < 2 && indicesPrefer.length < 2)
      return { board: currentBoard, active: currentActive };

    const [first, second] = (
      indicesPrefer.length >= 2 ? indicesPrefer.slice(0, 2) : shuffle(empties)
    ).slice(0, 2);
  ...
  function fillDistractorsFinite(currentBoard, currentActive) {
    ...
    while (empties.length && guard < totalCells * 3) {
      ...
      const wouldCompletePair =
        aIds.has(card.id) &&
        b2.some((t) => t && t.pairId === card.id && t.lang !== side);
      if (wouldCompletePair) {
        empties.unshift(i);
        continue;
      }
  ...
  return (
    <div className="vocab-memory-page">
                      ...
                      <Space>
                        ...
                        {tile.audio && tile.lang === LANG_EN && (
                          <Button
                            ...
                            onClick={(e) => {
                              e.stopPropagation();
                              new Audio(tile.audio).play().catch(() => {});
                            }}
                          />
                        )}
                      </Space>
      ...
      <div className="page-footer">
        <Text type="secondary">
          Mẹo: Chọn 1 ô English và 1 ô Vietnamese tạo thành cặp đúng để ghi
          điểm. Khi hết từ mới, trò chơi sẽ kết thúc và hiển thị kết quả.
        </Text>
      </div>
      ...
      >
        <Space direction="vertical" size={6}>
          <Text>
            ✅ Số cặp đúng: <b>{score}</b>
          </Text>
          <Text>
            🔁 Số lượt chọn: <b>{moves}</b>
          </Text>
        </Space>
      </Modal>
    </div>
