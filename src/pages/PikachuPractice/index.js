...
function countTilesById(boardArr) {
  const m = new Map();
  boardArr.forEach((t) => {
    if (t) m.set(t.pairId, (m.get(t.pairId) || 0) + 1);
  });
  return m; // 0/1/2
}
...
function tilesOfId(boardArr, id) {
  const idxs = [];
  boardArr.forEach((t, i) => {
    if (t && t.pairId === id) idxs.push(i);
  });
  return idxs; // 0,1,2 indices
}

function fillSinglesPreferSlotsLocal(
  boardIn,
  preferSlots = [],
  maxSlots = 2,
  pool,
...
function ensureOnePairLocal(
  ...
  for (const [id, cnt] of counts.entries()) {
    if (cnt === 1 && !a2.has(id)) {
      ...
      const slots = (preferSlots.length ? preferSlots : empties).filter(
        (i) => b2[i] === null,
      );
  ...
  const empties = emptyIndices(b2);
  const slots = (
    preferSlots.length >= 2 ? preferSlots.slice(0, 2) : empties
  ).slice(0, 2);
...
export default function PikachuPractice() {
  ...
  useEffect(() => {
    activeRef.current = activePairIds;
  }, [activePairIds]);
  useEffect(() => {
    poolRef.current = remainingIds;
  }, [remainingIds]);
  ...
  useEffect(() => {
    ...
    const pairsToSeed =
      Math.min(3, Math.floor(totalCells / 2), ids.length) ||
      (ids.length ? 1 : 0);
  ...
  function onTileClick(index) {
    ...
    if (first.pairId === second.pairId) {
      ...
      setTimeout(() => {
        ...
        let b2 = boardRef.current.map((t) =>
          t && t.pairId === pid ? null : t,
        );
        ...
        for (const [, cnt] of countsNow.entries()) {
          if (cnt === 2) {
            stillHasPair = true;
            break;
          }
        ...
        if (!stillHasPair) {
          const ensured = ensureOnePairLocal(
            b2,
            a2,
            originalRemoved,
            pool,
        ...
        if (removedSlotsNow.length && pool.length) {
          const fillRemoved = fillSinglesPreferSlotsLocal(
            b2,
            removedSlotsNow,
            removedSlotsNow.length,
            pool,
            allCards,
          );
          b2 = fillRemoved.board;
          pool = fillRemoved.pool;
        }

        // 🔥 7b) Fallback khi pool = 0: dùng chính các singleton hiện có để lấp slot
        const removedSlotsStillEmpty = removedSlotsNow.filter(
          (i) => b2[i] === null,
        );
        if (removedSlotsStillEmpty.length && pool.length === 0) {
          const completed = completeSingletonsIntoSlots(
            b2,
            removedSlotsStillEmpty,
            a2,
            allCards,
          );
          b2 = completed.board;
  ...
  function playAudio(url, fallbackText, langCode = "en-US") {
    ...
    if (window.location.protocol === "https:" && url.startsWith("http:")) {
      ...
      if ("speechSynthesis" in window && fallbackText) {
        ...
        message.warning(
          "Audio bị chặn do mixed content (http). Hãy dùng URL https.",
        );
      }
      return;
    }
                  ...
                  {tile ? (
                    ...
                    >
                      <Text className="tile-label">
                        {tile.lang === LANG_EN ? "EN" : "VI"}
                      </Text>
                      <Text
                        className={`tile-text ${
                          tile?.lang === LANG_EN ? "title-EN-text" : ""
                        }`}
                      >
                        {tile.text}
                      </Text>
                      <Space>
                        ...
                        {tile.audio && tile.lang === LANG_EN && (
                          <Button
                            ...
                            onClick={(e) => {
                              e.stopPropagation();
                              playAudio(tile.audio, tile.text, "en-US");
                            }}
                          />
                        )}
                      </Space>
            ...
            <aside className="sidebar">
              <div className="side-card side-howto">
                ...
                <ul className="side-steps">
                  <li>
                    Chọn <b>1 ô EN</b> và <b>1 ô VI</b> để tạo <b>cặp đúng</b>.
                  </li>
                </ul>
              </div>
        ...
        <div style={{ textAlign: "center" }}>
          ...
          >
            <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
              ...
              <span>Số cặp đúng: {score}</span>
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
              <CiRepeat style={{ fontSize: "26px", color: "red" }} />
              <span>Số lượt chọn: {moves}</span>
            </div>
          </div>
          ...
          >
            ...
            <Button className="OptionForm__button" onClick={() => navigate(-1)}>
